...
public class FastScroller extends RecyclerView.ItemDecoration implements RecyclerView.OnItemTouchListener {
    ...
    private final Drawable mHorizontalThumbDrawable;
    ...
    private final RecyclerView.OnScrollListener
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            updatePopupText(recyclerView);
        }
    };

    public FastScroller(RecyclerView recyclerView, Drawable verticalThumbDrawable,
                        Drawable verticalTrackDrawable, Drawable horizontalThumbDrawable,
                        Drawable horizontalTrackDrawable, View popupView, int defaultWidth, int scrollbarMinimumRange,
                        int margin) {
    ...
    private void drawVerticalScrollbar(Canvas canvas) {
        ...
        if (isLayoutRTL()) {
            ...
            mVerticalThumbDrawable.draw(canvas);
            if (mDragState == DRAG_Y && mPopupView != null && !TextUtils.isEmpty(getPopupText())) {
                layoutPopupView();
                int popupRight = mPopupWidth;
                int popupTop = mVerticalThumbHeight / 2 - mPopupHeight / 2;
                canvas.translate(-popupRight, popupTop);
                mPopupView.draw(canvas);
                canvas.translate(-left + popupRight, -top - popupTop);
            } else {
    ...
    private void layoutPopupView() {
        ...
        if (!(popupWidth == mPopupWidth && popupHeight == mPopupHeight)) {
            mPopupView.layout(0, 0, popupWidth, popupHeight);
...
//                    (verticalVisibleLength * verticalVisibleLength) / verticalContentLength);
            mVerticalThumbHeight = mVerticalThumbDrawable.getIntrinsicHeight();
    ...
    private void updatePopupText(RecyclerView recyclerView) {
        ...
        if (adapter instanceof PopupTextProvider) {
            ...
            String popupText = ((PopupTextProvider) adapter).getPopupText(position);
            setPopupText(popupText);
        }
    }

    private void setPopupText(String text) {
        if (mPopupView instanceof PopupTextHolder) {
            ((PopupTextHolder) mPopupView).setPopupText(text);
        }
    }

    private String getPopupText() {
        if (mPopupView instanceof PopupTextHolder) {
            return ((PopupTextHolder) mPopupView).getPopupText();
        }
        return "";
    }
    ...
    private class AnimatorUpdater implements AnimatorUpdateListener {
        ...
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
            ...
            if (mDragState == DRAG_Y && !TextUtils.isEmpty(getPopupText())) {
                mPopupView.setAlpha(alpha);
            }
            requestRedraw();
        }
    }
